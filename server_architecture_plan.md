...
> **方針:** 具体的なスレッド番号（`cpuset-cpus` の値）は、各サービスの `docker-compose.yml` を作成する段階で決定する。
> **⚠️ 構築時の重要事項:** Ubuntuインストール直後にターミナルで `lscpu -e` を実行し、どのCPUスレッドIDがどのコア（Pコア・Eコア）にマッピングされているかを必ず実機で確認してから、`docker-compose.yml` に `cpuset-cpus` を記述すること。
> ここでは割当の基本概念のみを定め、追加サービスが増えるたびに柔軟に変更できる運用とする。

| コアの種類 | 担当する処理の方針 |
| :--- | :--- |
| **Pコア** | 高負荷・リアルタイム性が命の処理（映像配信・エンコード等） |
| **Eコア** | バックグラウンド・軽負荷処理（Bot、同期処理、補助サービス等） |
